...
<html>
    ...
    <?php } ?>
        ...
        <div style="width: 100%; text-align: center">
            <p>LAPORAN KEMAJUAN PELAKSANAAN KEGIATAN</p>
            <p>DANA ALOKASI KHUSUS (DAK) FISIK <?=strtoupper(@$data[0]['jenis_nama'])?></p>
            <!-- <p>DANA ALOKASI KHUSUS (DAK) FISIK REGULER/PENUGASAN/AFIRMASI *}</p> -->
            <p>BIDANG <?=strtoupper(@$bidang_nama)?> </p>
                ...
                <?php
                        ...
                        foreach ($data as $row){
                            if($row['level'] <= 4){
                                ...
                                $uang = $row['detail_rincian_tw'.$triwulan.'_keuangan_rp'];
                                if($row['detail_rincian_pelaksanaan_jenis'] == 1){
                                    $pagu = $row['detail_rincian_swakelola_rp'];
                                }else{
                                    $pagu = $row['detail_rincian_konstraktual_rp'];
                                }
                                // $pagu = $row['detail_rincian_pagu'];
                            }

                            $fisikPersen = @$row['persen_jumlah'];
                            $keuanganPersen = @$row['keuangan_persen'];
                            if($pagu > 0){
                            ...
                            if(count($keuanganPersen) > 0){
                                $jumKeuanganPersen = array_sum($keuanganPersen)/count($keuanganPersen);
                            }else{
                                $jumKeuanganPersen = 0;
                            }
                            ...
                            if($row['level'] == 1){
                                ...
                                <tr>
                                    ...
                                    <td <?=$style?> ></td>
                                    <td <?=$style?> ></td>
                                    <td <?=$style?> ><?=number_format($row['pagu_all_jumlah'],2,",",".")?></td>
                                    <td <?=$style?> ></td>
                                    ...
                                    <td <?=$style?> ><?=number_format($jumKeuanganPersen,2,",","")?></td>
                                    <td <?=$style?> ></td>
                                    <td <?=$style?> ><?=number_format($jumFisikPersen,2,",","")?></td>
                                    <td <?=$style?> ></td>
                                </tr>
                                <?php
                                $noKeg = 1;
                            ...
                            <tr>
                                ...
                                <td <?=$style?> ><?=number_format($uang,2,",",".")?></td>
                            ...
                            for($k = 0; $k < count(@$row['dataPenunjang']); $k++){
                                ...
                                if($k == 0){
                                    ...
                                    <tr>
                                        ...
                                        <td <?=$style?> ></td>
                                        <td <?=$style?> ></td>
                                        <td <?=$style?> ><?=number_format(@$row['dataPenunjang'][$k]['pagu_all_jumlah'],2,",",".")?></td>
                                        <td <?=$style?> ></td>
                                        ...
                                        <td <?=$style?> ><?=number_format(COUNT($row['dataPenunjang'][$k]['keuangan_persen'])>0?array_sum($row['dataPenunjang'][$k]['keuangan_persen'])/COUNT($row['dataPenunjang'][$k]['keuangan_persen']):0,2,",","")?></td>
                                        <td <?=$style?> ></td>
                                        <td <?=$style?> ><?=number_format($jumFisikPersen,2,",","")?></td>
                                        <td <?=$style?> ></td>
                                    </tr>
                                    ...
                                    $uang = $row['dataPenunjang'][$k]['kegiatan_penunjang_tw'.$triwulan.'_keuangan_rp'];
                                    if($row['dataPenunjang'][$k]['kegiatan_penunjang_pelaksanaan_jenis'] == 1){
                                        $pagu = $row['dataPenunjang'][$k]['kegiatan_penunjang_swakelola_rp'];
                                    }else{
                                        $pagu = $row['dataPenunjang'][$k]['kegiatan_penunjang_konstraktual_rp'];
                                    }
                                    if($pagu > 0){
                                        $persen = 100*($uang/$pagu); 
                                    }else{
                    ...
                    if(count($arrPersen) > 0){
                        $totalPersen = array_sum($arrPersen)/count($arrPersen);
                    }else{
                        $totalPersen = 0;
                    }
                    if(count($totalUangPersen) > 0){
                        $jumTotalUangPersen = array_sum($totalUangPersen)/count($totalUangPersen);
                    }else{
                        $jumTotalUangPersen = 0;
                    }
                ?>
                <tr>
                    <td colspan="8" style="<?=$styleEdit?> text-align: right;">TOTAL</td>
                    <td <?=$style?> ><?=number_format($totalAll,2,",",".")?></td>
                    <td <?=$style?> ></td>
                    <td <?=$style?> ><?=number_format($totalSwakelola,2,",",".")?></td>
                    <td <?=$style?> ></td>
                    <td <?=$style?> ><?=number_format($totalKonstraktual,2,",",".")?></td>
                    <td <?=$style?> ></td>
                    <td <?=$style?> ><?=number_format($totalKeuangan,2,",",".")?></td>
                    <td <?=$style?> ><?=number_format($jumTotalUangPersen,2,",",".")?></td>
                    <td <?=$style?> ></td>
                    <td <?=$style?> ><?=number_format($totalPersen,2,",",".")?></td>
                    <td <?=$style?> ></td>
                </tr>
            </table>
        </div>
        <div style="height: 220px; width: 100%;">
           ...
           <br>
           <br>
           <img src="<?=base_url()."public/images/ttd.jpg"?>" alt="ttd">
            <table>
                <tr>
                    <td></td>
                    <td>Lasusua2, ..........................</td>
